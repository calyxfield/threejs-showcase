...
export function validateSave(save) {
  if (!save || ![1, 2, 3, 4, 5, 6, 7, 8].includes(save.version) || ![LEGACY_GENERATOR, CURRENT_GENERATOR].includes(save.generator) || (save.version === 1 && save.generator !== LEGACY_GENERATOR)) throw new Error('This save uses an unsupported world version.');
...
export function encodeSave(id, name, world, camera, zoom) {
  ...
  return validateSave({ version: 8, generator: world.generator, id, name, updated: Date.now(), settings: { ...world.settings }, camera: { ...camera }, zoom, edits, economy: world.economy ? structuredClone(world.economy) : null, construction: world.construction ? structuredClone(world.construction) : null });
}
