...
export const GENERATOR = 'woodland-perlin-v1';
const finite = n => typeof n === 'number' && Number.isFinite(n);
const text = (s, max) => typeof s === 'string' && s.length <= max;

export function validateSave(save) {
  if (!save || save.version !== 1 || save.generator !== GENERATOR) throw new Error('This save uses an unsupported world version.');
  ...
  for (const edit of edits) if (!Array.isArray(edit) || edit.length !== 3 || !Number.isSafeInteger(edit[0]) || !Number.isSafeInteger(edit[1]) || !Number.isInteger(edit[2]) || edit[2] < 0 || edit[2] > 63) throw new Error('A saved world has invalid tile changes.');
  return save;
}
export function encodeSave(id, name, world, camera, zoom) {
  ...
  return validateSave({ version: 1, generator: GENERATOR, id, name, updated: Date.now(), settings: { ...world.settings }, camera: { ...camera }, zoom, edits });
}
