...
export function validateSave(save) {
  if (!save || ![1, 2].includes(save.version) || ![LEGACY_GENERATOR, CURRENT_GENERATOR].includes(save.generator) || (save.version === 1 && save.generator !== LEGACY_GENERATOR)) throw new Error('This save uses an unsupported world version.');
  ...
  if (save.version === 2 && (save.generator === CURRENT_GENERATOR || save.construction !== null)) validateConstruction(save.construction);
  return save;
}
export function encodeSave(id, name, world, camera, zoom) {
  ...
  return validateSave({ version: 2, generator: world.generator, id, name, updated: Date.now(), settings: { ...world.settings }, camera: { ...camera }, zoom, edits, construction: world.construction ? structuredClone(world.construction) : null });
}
